...
void CameraManager::Init(LevelData* level, Camera* camera,
	...
	state_->init[int(BehaviorGS::Movie)] = [this]([[maybe_unused]] const BehaviorGS& pre) {
	...
	state_->update[int(BehaviorGS::Movie)] = [this]([[maybe_unused]] std::optional<BehaviorGS>& req, [[maybe_unused]] const BehaviorGS& pre) {
		// 曲線の処理を行う
