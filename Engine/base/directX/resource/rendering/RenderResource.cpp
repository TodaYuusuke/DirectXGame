...
void RenderResource::InitUAV(){
	...
	// 2．クリアカラー
	clearValue.Format = DXGI_FORMAT_R32_SINT;
	clearValue.Color[0] = -1.0;
	...
	srvInfo = SRV::GetInstance()->CreateRenderResource(resource_.Get());
	// UAV上に登録
	uavInfo = SRV::GetInstance()->CreateRenderResourceUAV(resource_.Get());
}
void RenderResource::InitStencilMask() {
	HRESULT hr = S_FALSE;
