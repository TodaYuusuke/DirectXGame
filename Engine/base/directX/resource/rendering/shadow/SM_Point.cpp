...
void SM_Point::Init(GPUDevice* device, HeapManager* heaps) {
	HRESULT hr = S_FALSE;
	...
	// SRV上に登録
	srvInfo = heaps->srv()->CreateShadowMapPoint(resource_.Get());
	// DSV上に登録
	std::array<DSVInfo, 6> data = heaps->dsv()->CreateShadowMapPoint(resource_.Get());
