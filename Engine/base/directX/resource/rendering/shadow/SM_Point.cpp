...
void SM_Point::Init() {
	...
	// 解像度
	width = static_cast<int>(1024.0f * lwpC::Shadow::Point::kResolution);
	height = static_cast<int>(1024.0f * lwpC::Shadow::Point::kResolution);

	// 1. Resourceの設定
	desc.Width = width; // Textureの幅
	desc.Height = height; // Textureの高さ
	desc.MipLevels = 1; // mipmapの数
	desc.DepthOrArraySize = 6; // キューブマップなので6
	...
	// SRV上に登録
	srvInfo = SRV::GetInstance()->CreateShadowMapPoint(resource_.Get());
	// DSV上に登録
	std::array<DSVInfo, 6> data = DSV::GetInstance()->CreateShadowMapPoint(resource_.Get());
	dsvInfo = std::move(data[0]);
	for (int i = 0; i < 5; i++) {
		dsvInfos[i] = std::move(data[i + 1]);
	}
}

void SM_Point::Clear(ID3D12GraphicsCommandList* list) {
	// 指定した深度で画面全体をクリアする
