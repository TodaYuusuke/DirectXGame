...
void MainRenderer::Init(ID3D12Device* device, ID3D12RootSignature* root, ID3D12PipelineState* pso, HeapManager* heaps) {
	// ルートシグネチャのポインタ
	root_ = root;
	// PSOのポインタ
	pso_ = pso;
...
void MainRenderer::Draw(ID3D12GraphicsCommandList* list, int instanceCount) {
	PreDraw(list);
	...
	// ディスクリプタテーブルを登録
	list->SetGraphicsRootDescriptorTable(0, viewStruct_->index);
...
void MainRenderer::SetRenderData(Resource::RenderTexture* target, Math::Matrix4x4 vp) {
	// レンダリング用のデータを登録
	renderData_->target = target;
	*renderData_->vpResource->data_ = vp;
}

void MainRenderer::PreDraw(ID3D12GraphicsCommandList* list) {
	// 書き込み先のリソースを取得
	RenderResource* rr = renderData_->target->GetRenderResource();
...
void MainRenderer::PostDraw(ID3D12GraphicsCommandList* list) {
	...
	D3D12_RESOURCE_BARRIER barrier0 = ICommand::MakeResourceBarrier(
		renderData_->target->GetTexResource(),
	...
	list->CopyResource(renderData_->target->GetTexResource(), rr->GetResource());
