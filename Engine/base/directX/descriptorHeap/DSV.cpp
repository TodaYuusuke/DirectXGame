...
ID3D12Resource* DSV::CreateDirectionShadowMap(uint32_t* dsvIndex, D3D12_GPU_DESCRIPTOR_HANDLE* view) {
	...
	// DSVResourceの設定
	D3D12_RESOURCE_DESC resourceDesc{};
	resourceDesc.Width = 1024 * static_cast<int>(lwpC::Shadow::kResolutionScale); // Textureの幅
	resourceDesc.Height = 1024 * static_cast<int>(lwpC::Shadow::kResolutionScale); // Textureの高さ
	resourceDesc.MipLevels = 1; // mipmapの数
	resourceDesc.DepthOrArraySize = 1; // 奥行き or 配列Textureの配列数
