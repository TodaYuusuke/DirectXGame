...
Sphere::Sphere() : Sphere(0.5f) {};
Sphere::Sphere(const float& rad) {
	radius = rad;
...
void Sphere::GetBoundingAABB(Vector3* minPtr, Vector3* maxPtr) {
	...
	Vector3 center = tf_->GetWorldPosition();
...
void Sphere::Create(LWP::Resource::RigidModel* model) {
	...
	// ここから中心点を割り出す
	tf_->translation = (max + min) / 2.0f;
	// 半径を割り出す
	radius = (max - tf_->translation).Length();
}
...
Sphere::Data::Data(Sphere& sphere) {
	position = sphere.tf_->GetWorldPosition();
